...
class ServerWorkerThread : Thread {
     ...
     */
    private void run() {
        this.logger.debug_("Worker started.");
        while (server.isReady) {
            try {
                ...
                this.logger.error("An unhandled exception occurred in this worker's `run` method.", e);
            }
        }
        this.logger.debug_("Worker stopped normally after server was stopped.");
    }
...
package class PoolManager : Thread {
    ...
    private void checkPoolHealth() {
        this.logger.debug_("Checking worker pool health.");
        uint busyCount = 0;
        uint waitingCount = 0;
        uint deadCount = 0;
        synchronized(this.pool.workersMutex.writer) {
            for (size_t idx = 0; idx < this.pool.workers.length; idx++) {
                ...
                if (!worker.isRunning()) {
                    ...
                    ServerWorkerThread newWorker = new ServerWorkerThread(
                        this.pool.server, this.pool.requestQueue, this.pool.nextWorkerId++
                    );
                    newWorker.start();
